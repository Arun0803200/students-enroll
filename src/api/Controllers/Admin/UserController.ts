...
export class UserController {
  ...
  public async createUser(
    ...
    if (ifEmail) {
      return response.status(400).send({
        status: 0,
        message: 'The given email already exists. Please try another email :)',
      });
    }
  ...
  public async getEmployee(
    @QueryParam('limit') limit: number,
    @QueryParam('offset') offset: number,
    @QueryParam('count') count: boolean | number,
    ...
    if (count) {
      ...
      return response.status(200).send({
        status: 1,
        message: 'Successfully got the count of employee',
        data: countData,
      });
    }
    if (keywords) {
    ...
    if (keywords) {
      ...
      matchCondition = {
        ...
        $or: [
          ...
          {
            'roledetails.roleName': regexp,
          },
        ],
      };
    }
    const employeeDatas = await employee.aggregate([
      {
        $lookup: {
          from: 'employeeRole',
          localField: '_id',
    ...
    return response.status(200).send({
      status: 1,
      message: 'Successfully retrive the employee data',
      data: employeeDatas,
    });
  }
}
